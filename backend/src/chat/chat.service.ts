...
import { MemuService } from '../memu/memu.service';
...
export class ChatService {
  ...
  constructor(
    ...
    private readonly memuService: MemuService,
  ...
  async *streamChat(chatRequest: ChatRequestDto): AsyncGenerator<string> {
    ...
    let systemPrompt = this.replaceVariables(
      character.systemPrompt,
      chatRequest,
    );

    // ✅ memU 记忆注入：检索相关记忆并注入到 System Prompt
    let memoriesText = '';
    if (this.memuService.isEnabled() && chatRequest.sessionKey) {
      try {
        const memories = await this.memuService.retrieve(
          chatRequest.sessionKey,
          chatRequest.characterId,
          chatRequest.message,
          'rag', // 使用快速 RAG 检索
        );
        if (memories && memories.items.length > 0) {
          memoriesText = this.memuService.formatMemoriesForPrompt(
            memories.items,
          );
          systemPrompt = systemPrompt + '\n' + memoriesText;
          this.logger.log(`memU: 注入 ${memories.items.length} 条记忆到上下文`);
        }
      } catch (error) {
        this.logger.warn('memU 记忆检索失败，继续对话', error.message);
      }
    }
    ...
    if (chatRequest.history && chatRequest.history.length > 0) {
      const maxTurns = parseInt(
        this.configService.get<string>('MAX_HISTORY_TURNS') || '20',
      );
      ...
      if (originalLength > maxMessages) {
        this.logger.log(
          `上下文窗口截断: ${originalLength} -> ${truncatedHistory.length} (保留最近${maxTurns}轮)`,
        );
      }
    ...
    if (chatRequest.imageUrl) {
      ...
      try {
        const imageDescription = await this.getImageDescription(
    ...
    // 日志：记录最终发送的消息数量
    this.logger.log(
      `LLM请求 - 总消息数: ${messages.length} (system: 1, history: ${messages.length - 2}, current: 1)`,
    );
    ...
    if (!modelConfig) {
      ...
      if (!fallbackApiKey || !fallbackBaseURL) {
        throw new BadRequestException(
          `模型 "${modelId}" 未配置，且环境变量fallback不可用`,
        );
      }
      ...
      const decryptedApiKey =
        this.modelsService.getDecryptedApiKey(modelConfig);
      this.openai = new OpenAI({
    ...
    // ✅ 使用角色配置的模型和参数
    const temperature =
      character.temperature ?? modelConfig?.defaultTemperature ?? 0.7;
    const maxTokens =
      character.maxTokens ?? modelConfig?.defaultMaxTokens ?? 2000;

    this.logger.log(
      `模型配置 - model: ${modelId}, temperature: ${temperature}, maxTokens: ${maxTokens}`,
    );
    ...
    if (totalTokens > 0) {
      this.logger.log(
        `Token统计 - Prompt: ${promptTokens}, Completion: ${completionTokens}, Total: ${totalTokens}`,
      );
    } else {
    ...
    // ✅ memU 记忆存储：对话完成后异步存储记忆
    if (
      this.memuService.isEnabled() &&
      chatRequest.sessionKey &&
      responseText
    ) {
      // 异步存储，不阻塞对话流程
      setImmediate(async () => {
        try {
          ...
          this.logger.log('memU: 记忆存储任务已提交');
        } catch (error) {
          this.logger.warn('memU 记忆存储失败', error.message);
        }
      });
    }
  }
  ...
  private async getImageDescription(imageUrl: string): Promise<string> {
    ...
    if (
      ...
      try {
        ...
        this.logger.log(
          `图片已转换为 base64 格式 (${(base64Image.length / 1024).toFixed(2)}KB)`,
        );
      } catch (error) {
    ...
    try {
      this.logger.log(
        `调用 GLM API - modelId: ${glmModelId}, imageData length: ${imageData.length}`,
      );
        ...
        messages: [
          {
            ...
            content: [
              ...
              {
                type: 'text',
                text: '请详细描述这张图片的内容，包括主要物体、场景、颜色、氛围等细节。',
              },
            ],
          },
        ],
      ...
      const description =
        message?.reasoning_content || message?.content || '无法识别图片内容';
      ...
      throw new BadRequestException(
        `图片识别失败：${error.status || ''} ${error.message}`,
      );
    }
  }
