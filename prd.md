...
*   **角色管理 (Admin)**：
    ...
    *   Few-shot 示例对话：支持录入多组示例问答，用于增强人设稳定性。
    *   模型配置：支持为角色选择模型与温度等参数（仅展示可用模型）。
    *   删除角色：支持移除不再使用的角色。
...
*   **上下文记忆**：
    ...
    *   记录 Token 使用量（流式优先读取 usage，无法获取时可估算）。
*   **图片交互**：
    *   用户可上传本地图片。
    *   后端接收图片并存储（本地或 OSS）。
...
*   **对话历史管理**：
    ...
    *   支持删除与导出会话数据。
*   **流式体验优化**：
    *   仅当用户位于底部时自动滚动；用户上滑时显示“新消息”提示。
    *   对流式输出进行防抖/缓冲，避免 Markdown 未闭合导致抖动。
...
*   **鉴权与限流**：管理类接口支持简单鉴权（如 `X-Admin-Secret`）并可限流。
*   **错误恢复机制**：SSE 中断后提供重试或恢复路径。
...
*   **决策**：设计通用的 **LLM Service** 接口，支持多厂商接入与角色级模型选择。默认配置支持 OpenAI SDK 格式，可扩展接入 DeepSeek、Claude、Gemini、Qwen、GLM（需适配各自 SDK 或 OpenAI-compatible 网关）。
