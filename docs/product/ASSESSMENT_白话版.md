...
我开发了一个新功能（比如模型管理模块），开发完测试都正常，高高兴兴地提交了代码。但第二天早上打开系统一看——**原来的角色管理功能全部消失了**！

- 角色列表页面：空的
- 模型管理页面：也不见了
- 聊天历史：还在，但无法创建新角色

**吓冷汗的原因**：角色和模型是同一个模块，修改某个文件时，覆盖了原有的代码。

**怎么排查的？**

还好我**一直有使用git的好习惯**：
1. 每天工作结束前都会 `git commit` 提交代码
2. 重要节点打 `git tag` 标记版本
3. 每个大功能开发前会创建新分支
...
# 找到问题功能还在的最后一次提交
# 比如：66184d6d docs(assessment): 补充完善问题解决案例

# 回退到那个版本
git reset --hard 66184d6d
...
1. **代码覆盖**：新功能的某个文件覆盖了原有文件，没有合并，直接替换了
2. **分支管理不当**：应该在独立分支开发，然后merge回主分支
3. **测试不全面**：只测了新功能，没验证原有功能是否还正常

**经验教训：**

1. **git是救命稻草**：如果没有版本控制，这些代码就真的丢了！
2. **开发前必须备份**：大功能开发前先打tag：`git tag v1.0-backup`
3. **使用分支开发**：不要直接在主分支上大改
4. **测试要全面**：开发新功能时，同时测试原有功能有没有被破坏
5. **提交要频繁**：每天结束前提交，避免丢失太多工作

**为什么这个案例重要？**

答辩时可以这样说：

> "这个问题让我深刻理解了版本控制的重要性。我养成了每天提交代码的习惯，git真的救了我！如果没有git，那些代码就真的丢了。这让我学会了：开发新功能时一定要注意不要破坏原有功能，测试要全面，及时提交备份。"
